...
import { validateOtp } from '@/composables/validate/useAuthValidate'
import { useRoute, useRouter } from "vue-router"
import { useForgotPasswordReSendOtp, useVerifyForgotPasswordOtp } from '@/hooks/queries/auth/useForgotPassword'

const route = useRoute()
const emailUrl = computed(() => route.query.email as string);
...
const isSubmitDisabled = computed(() => !!validateOtp(otp.value))
const { mutate: verifyOtpMutation } = useVerifyForgotPasswordOtp();
const { mutate: forgotPasswordReSendOtpMutaion } = useForgotPasswordReSendOtp();

const submit = () => {
    const error = validateOtp(otp.value)
...
const resendOtp = () => {
    forgotPasswordReSendOtpMutaion(emailUrl.value, {
        onSuccess: () => {
            otp.value = ''
            otpError.value = ''
...
<template>
    <div class="min-h-screen relative overflow-hidden bg-[#f5efe2] text-[#234030]">
        ...
        <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div class="w-170 h-170 rounded-full border border-[#b89243]/20"></div>
            <div class="absolute w-140 h-140 rounded-full border border-[#2f5a43]/15"></div>
            <div class="absolute w-110 h-110 rounded-full border border-[#b89243]/10"></div>
        </div>
                ...
                <div class="hidden lg:flex flex-col justify-center pr-8">
                    <div class="mb-8">
                        ...
                        <p class="mt-6 text-xl leading-relaxed text-[#49624f] max-w-xl">
                            Nhập mã OTP đã được gửi tới email của bạn để xác nhận yêu cầu khôi phục và tiếp tục đặt lại
                            mật khẩu.
                        </p>
                    </div>
                    ...
                    <div class="space-y-4 text-[#5b6c5f]">
                        <div class="flex items-center gap-3">
                            ...
                            <span>Xác minh đúng chủ tài khoản trước khi đặt lại mật khẩu</span>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="w-2.5 h-2.5 rounded-full bg-[#2f5a43]"></span>
                            <span>Bảo vệ dữ liệu gia phả và tư liệu của dòng họ</span>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="px-8 md:px-12 py-10 md:py-12">
                            <div class="flex flex-col items-center text-center">
                                ...
                                <p class="mt-3 text-[#5c6e61] max-w-md leading-relaxed">
                                    Vui lòng nhập mã OTP gồm 7 ký tự chữ và số, ví dụ `G1MCXOZ`, được gửi đến email khôi
                            ...
                            <form class="space-y-5" @submit.prevent="submit">
                                <div>
                                    <label class="block mb-3 text-sm font-medium text-[#355340] text-center">
                                        Mã OTP
                                    </label>
                                    <div
                                        class="rounded-[28px] border border-[#c8b27a]/45 bg-linear-to-br from-[#fffaf1] to-[#f8efdf] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.75),0_14px_34px_rgba(76,57,22,0.08)] transition focus-within:border-[#2f5a43] focus-within:shadow-[0_0_0_4px_rgba(47,90,67,0.08),0_18px_42px_rgba(76,57,22,0.12)]">
                                        <div class="flex items-center justify-between gap-3 px-2 pb-3">
                                            <div class="flex items-center gap-2 text-[#7a6843]">
                                                <div
                                                    class="flex h-9 w-9 items-center justify-center rounded-full bg-[#234030]/8 border border-[#c8b27a]/40">
                                                    <KeyRound class="w-4 h-4" />
                                                </div>
                                                <span class="text-xs uppercase tracking-[0.24em] font-semibold">
                                                    Security Code
                                                </span>
                                            </div>
                                            <span
                                                class="rounded-full border border-[#c8b27a]/40 bg-white/70 px-3 py-1 text-[11px] font-medium text-[#6d5a35]">
                                                7 ký tự
                                            </span>
                                        </div>

                                        <input v-model="otp" type="text" maxlength="7" placeholder="otp"
                                            class="w-full rounded-2xl border border-[#e2d2aa]/70 bg-white/65 px-5 py-4 text-center text-2xl font-semibold tracking-[0.38em] text-[#234030] uppercase outline-none placeholder:tracking-[0.32em] placeholder:text-[#b39a68]" />
                                    </div>

                                    <p v-if="otpError" class="mt-2 text-sm text-center text-red-600">
                                        {{ otpError }}
                                    </p>
                                </div>

                                <div class="rounded-2xl border border-[#c8b27a]/40 bg-[#fff9ed]/70 p-4">
                                    <div class="flex items-start gap-3">
                                        <ShieldCheck class="w-5 h-5 text-[#7a6843] mt-0.5 shrink-0" />
                                        <p class="text-sm leading-relaxed text-[#5a6b5f]">
                                            Mã OTP chỉ có hiệu lực trong thời gian ngắn và có thể bao gồm cả chữ lẫn số.
                                            Nếu chưa nhận được mã, bạn có thể yêu cầu gửi lại.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <button type="submit" :disabled="isSubmitDisabled"
                                    ...
                                    <span class="relative z-10">Xác nhận mã OTP</span>
                                    <span
                                ...
                                <p v-if="isSubmitted" class="text-sm text-center text-[#2f5a43]">
                                    Mã OTP hợp lệ. Bạn có thể tiếp tục bước đặt lại mật khẩu.
                                </p>

                                <p v-if="isResent" class="text-sm text-center text-[#2f5a43]">
                                    Mã OTP mới đã được gửi lại. Vui lòng kiểm tra email của bạn.
                                </p>
                            </form>

                            <div class="mt-8 text-center space-y-3">
                                <button type="button"
                                    class="text-[#7d6840] hover:text-[#234030] transition font-medium cursor-pointer"
                                    @click="resendOtp">
                                    Gửi lại mã OTP
                                </button>
                                <p class="text-[#607264]">
