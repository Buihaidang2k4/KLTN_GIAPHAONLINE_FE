<template>
  <div class="p-6 bg-slate-50 min-h-screen font-sans">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
      <!-- Biểu đồ biến động (Giả lập bằng SVG) -->
      <div class="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        ...
        <div class="h-64 w-full bg-slate-50 rounded-xl border border-dashed border-slate-200 flex flex-col items-center justify-center relative overflow-hidden">
           <!-- Một đường SVG vẽ tay để giả lập biểu đồ -->
          ...
          <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-50">
              <tr v-for="tx in transactions" :key="tx.id" class="hover:bg-slate-50/50">
                <td class="px-6 py-4">
                  <span class="text-xs font-mono text-amber-700">#{{ tx.id }}</span>
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 text-right">
                  <span class="text-sm font-black text-slate-800">{{ tx.amount.toLocaleString() }}đ</span>
                </td>
...
<style scoped>
/* Hiệu ứng mờ cho SVG Background của Stats */
.bg-amber-600 { background-color: #d97706; }
.bg-blue-600 { background-color: #2563eb; }
.bg-emerald-600 { background-color: #059669; }
.bg-rose-600 { background-color: #e11d48; }

/* Font mono cho mã giao dịch */
.font-mono {
