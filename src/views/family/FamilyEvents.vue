...
<template>
  <div class="p-8 bg-slate-50/50 min-h-screen">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
      ...
      <button class="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl transition-all shadow-lg shadow-indigo-200 active:scale-95 font-bold text-sm">
        <Plus :size="18" />
        Tạo sự kiện
      </button>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-slate-50/50 border-b border-slate-200">
              <th class="px-6 py-4 text-[13px] font-black text-slate-500 uppercase tracking-wider">Sự kiện</th>
              <th class="px-6 py-4 text-[13px] font-black text-slate-500 uppercase tracking-wider">Phân loại</th>
              <th class="px-6 py-4 text-[13px] font-black text-slate-500 uppercase tracking-wider">Thời gian</th>
              <th class="px-6 py-4 text-[13px] font-black text-slate-500 uppercase tracking-wider">Địa điểm</th>
              <th class="px-6 py-4 text-[13px] font-black text-slate-500 uppercase tracking-wider text-center">Trạng thái</th>
              <th class="px-6 py-4 text-[13px] font-black text-slate-500 uppercase tracking-wider"></th>
            </tr>
          </thead>
            ...
            >
              <td class="px-6 py-4">
                <div class="flex flex-col">
                  <span class="font-bold text-slate-800 group-hover:text-indigo-600 transition-colors">{{ event.title }}</span>
                  <span class="text-xs text-slate-400 mt-0.5 line-clamp-1 max-w-[200px]">{{ event.description }}</span>
                </div>
              </td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-2 text-sm text-slate-600 max-w-[180px]">
                  <MapPin :size="14" class="text-slate-400 shrink-0" />
                  <span class="truncate">{{ event.location }}</span>
                </div>
              </td>
              <td class="px-6 py-4 text-center">
                <span :class="['px-2.5 py-1 rounded-full text-[11px] font-black uppercase', getStatusClass(event.status)]">
                  {{ event.status === 'upcoming' ? 'Sắp tới' : (event.status === 'completed' ? 'Đã xong' : 'Đang diễn ra') }}
                </span>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-white rounded-lg border border-transparent hover:border-indigo-100 shadow-sm transition-all" title="Chi tiết">
                    <Info :size="18" />
                  </button>
                  <button class="p-2 text-slate-400 hover:text-slate-800 hover:bg-white rounded-lg border border-transparent hover:border-slate-200 shadow-sm transition-all">
      ...
      <div class="px-6 py-4 bg-slate-50/30 border-t border-slate-100 flex items-center justify-between text-sm text-slate-500 font-medium">
        <span>Hiển thị {{ filteredEvents.length }} sự kiện</span>
        <div class="flex gap-2">
          <button class="px-3 py-1.5 border border-slate-200 rounded-lg hover:bg-white disabled:opacity-50">Trước</button>
          <button class="px-3 py-1.5 bg-white border border-indigo-200 text-indigo-600 font-bold rounded-lg shadow-sm">1</button>
