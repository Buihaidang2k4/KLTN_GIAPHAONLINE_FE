...
const toolbarConfig = {
    // Loại bỏ các phím không cần thiết nếu muốn, ở đây tôi để mặc định để full chức năng
    excludeKeys: [
        // 'fullScreen' // Bạn có thể ẩn nút toàn màn hình nếu giao diện đã quá hẹp
    ],
    insertKeys: {
        index: 5,
...
const editorConfig = {
    ...
    MENU_CONF: {
        // Cấu hình font chữ phong phú cho gia phả
        fontFamily: {
            fontFamilyList: [
        ...
        // Màu sắc phối hợp với tông Amber/Slate của hệ thống
        color: {
    ...
    <div class="editor-wrapper shadow-sm border border-slate-200 rounded-2xl overflow-hidden bg-white flex flex-col">
        ...
        <div
            ...
            <div class="flex gap-4">
                ...
                <span v-if="editorRef">Words: {{ editorRef.getText().length }}</span>
            </div>
...
<style scoped>
/* Tinh chỉnh UI cho giống một ứng dụng cao cấp */
.editor-wrapper {
...
/* Tùy chỉnh toolbar của wangEditor */
:deep(.w-e-toolbar) {
    padding: 4px;
