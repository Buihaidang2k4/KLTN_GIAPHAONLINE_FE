...
import { Download, X, FileText, Sparkles } from 'lucide-vue-next'
    ...
    <Teleport to="body">
        <Transition name="fade">
            <div v-if="show && media" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <!-- Modal Container -->
                <div class="relative w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden rounded-[2.5rem] bg-slate-900 shadow-[0_40px_100px_-45px_rgba(0,0,0,0.8)] border border-white/5 animate-in fade-in zoom-in duration-300">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="min-w-0">
                                <div class="flex items-center gap-2 mb-0.5">
                                    ...
                                    <span class="text-[9px] font-bold text-amber-500/70 uppercase tracking-widest">Xem tài liệu gia đình</span>
                                </div>
                                <h3 class="line-clamp-1 text-lg font-black text-white leading-tight">{{ media.title || 'Tài liệu dòng tộc' }}</h3>
                    ...
                    <!-- Media Content -->
                    <div class="flex-1 flex flex-col items-center justify-center bg-[radial-gradient(circle_at_center,rgba(251,191,36,0.03),transparent_70%)] p-4 md:p-8 overflow-hidden min-h-0">
                        <!-- PDF Preview -->
                        <template v-if="isPdfDocument(media.mimeType)">
                            <iframe :src="`${media.mediaUrl}#toolbar=1`" class="flex-1 w-full rounded-2xl border border-white/5 shadow-2xl bg-white/5" />
                        </template>

                        <!-- Other documents -->
                        <template v-else>
                            <div class="flex flex-col items-center justify-center gap-8 text-center max-w-sm">
                                <div class="relative">
                                    <div class="absolute inset-0 bg-amber-500/20 blur-3xl rounded-full"></div>
                                    <div class="relative flex h-28 w-28 items-center justify-center rounded-[2rem] bg-white/5 border border-white/10 text-amber-500 shadow-2xl">
                                        <FileText :size="56" />
                                    </div>
                                </div>
                                <div>
                                    <h3 class="text-2xl font-black text-white tracking-tight">{{ media.title || 'Tài liệu' }}</h3>
                                    <p class="mt-2 text-xs font-bold text-white/40 uppercase tracking-widest">
                                        {{ getFileExtension(media.mimeType) }} • {{ formatByte(media.fileSizeBytes) }}
                                    </p>
                                </div>
                                <a :href="media.mediaUrl" target="_blank" rel="noopener noreferrer"
                                    class="flex items-center justify-center gap-3 rounded-2xl bg-white text-slate-900 px-8 py-3.5 text-xs font-black uppercase tracking-widest transition-all hover:bg-amber-400 active:scale-95 shadow-xl shadow-white/5">
                                    <Download :size="18" />
                                    Tải xuống ngay
                                </a>
                            </div>
