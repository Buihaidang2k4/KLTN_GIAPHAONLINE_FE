...
const { data: partnersResponse } = usePartnersQuery(parentId, {
    enabled: computed(() => props.isOpen && !!parentId.value)
});
...
const schema = toTypedSchema(
    zod.object({
        ...
        phoneNumber: zod
            ...
            .optional()
            .or(zod.literal("")),
        originPlace: zod.string().max(255, "Quê quán không được vượt quá 255 ký tự").optional(),
        placeOfResidence: zod.string().max(255, "Nơi ở hiện tại không được vượt quá 255 ký tự").optional(),
        biography: zod.string().max(1000, "Tiểu sử không được vượt quá 1000 ký tự").optional(),
        graveLocation: zod.string().max(255, "Nơi an táng không được vượt quá 255 ký tự").optional(),
