...
import { useUpdateArticleCategoryMutation } from '@/hooks/queries/article_category/useArticleCategory'
import type { ArticleCategoryRes } from '@/types/article_category/article-category.types'
...
const props = defineProps<{
    show: boolean
    category: ArticleCategoryRes | null
}>()
...
const validationSchema = toTypedSchema(
    z.object({
        name: z.string().trim()
            ...
            .max(255, 'CATEGORY_NAME_TOO_LONG'),
...
const updateMutation = useUpdateArticleCategoryMutation()
...
const onSubmit = handleSubmit(async (values) => {
    ...
    try {
        await updateMutation.mutateAsync({
            articleCategoryId: props.category.articleCategoryId,
            data: {
                name: values.name,
        ...
        notify.success('Cập nhật danh mục bài viết thành công', 'Thành công')
        emit('success')
        handleClose()
    } catch (err: any) {
        notify.error(err.response?.data?.message || 'Có lỗi xảy ra khi cập nhật danh mục', 'Thất bại')
    }
})
</script>
                ...
                <div
                    ...
                    <div class="relative px-6 pt-8 pb-4 text-center">
                        <div
                            ...
                            <span class="text-[9px] font-bold text-amber-700 uppercase tracking-widest">Cập nhật</span>
                        </div>
                        ...
                        <p v-if="category" class="mt-1 text-xs text-slate-500 font-medium">
                            #{{ category.articleCategoryId }} - {{ category.slug }}
                        </p>
